 ...
 try
		   {
			   ...
			   //sql="select * from `member`, `member_card` where `memberid`='" + mail + "' AND `member`.`memberid` = `member_card`.`memberid`";
               ResultSet rs =con.createStatement().executeQuery(sql1);
               if(rs.next())
	           { 
                out.print("<script>alert(decodeURIComponent('註冊失敗！卡片已存在'));location.href='card.html'</script>");
				if(request.getParameter("Password") != request.getParameter("Password_1"))
				{
					out.print("<script>alert(decodeURIComponent('密碼兩次輸入不相符'));location.href='card.html'</script>");
				}
	           }
               else
	           {
